...
ground = Entity(
    model="plane", collider="box", scale=64, texture="grass", texture_scale=(4, 4)
)

editor_camera = EditorCamera(enabled=False, ignore_paused=True)
player = FirstPersonController(
    model="cube", z=-10, color=color.orange, origin_y=-0.5, speed=8, collider="box"
)
...
gun.muzzle_flash = Entity(
    parent=gun, z=1, world_scale=0.5, model="quad", color=color.yellow, enabled=False
)
