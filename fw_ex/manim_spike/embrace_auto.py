...
class MWCode(Scene):
    def construct(self):
        ...
        manage_yourself = Text("Embrace Automation", **text_kwargs)
        with_code = Text("With Python", **text_kwargs)
        manage_yourself.shift(UP)
        with_code.shift(DOWN)

        self.add(title, author, top_line, bottom_line)
        self.play(GrowFromCenter(manage_yourself), GrowFromCenter(with_code))
        self.wait(2.5)
